...
class CheckerTests(unittest.TestCase):
    ...
    def test_excluded_symlink_paths_are_not_read(self) -> None:
        with self.repo() as directory, tempfile.TemporaryDirectory() as outside_directory:
            root = Path(directory)
            outside = Path(outside_directory) / "outside.py"
            outside.write_text("x = 1\n", encoding="utf-8")
            excluded = root / "fixtures"
            excluded.mkdir()
            (excluded / "escape.py").symlink_to(outside)
            policy(root, '\nexclude = ["fixtures/**"]\n')
            self.assertTrue(check(root, mode="all")["ok"])

    def test_parent_symlinks_and_special_files_fail_closed(self) -> None:
        with self.assertRaisesRegex(PolicyError, "relative"):
            effective_line_count(Path("."))
        with self.repo() as directory, tempfile.TemporaryDirectory() as outside_directory:
            root = Path(directory)
            linked = root / "linked"
            linked.symlink_to(outside_directory, target_is_directory=True)
            with self.assertRaises(PolicyError):
                effective_line_count(linked / "outside.py")
            if hasattr(os, "mkfifo"):
                pipe = root / "pipe.py"
                os.mkfifo(pipe)
                with self.assertRaisesRegex(PolicyError, "regular file"):
                    effective_line_count(pipe)
